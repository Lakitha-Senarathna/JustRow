...
public class customRow extends AppCompatActivity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvTimeElapsed = findViewById(R.id.timeElapsedCustom);
        tvDistance = findViewById(R.id.distanceCustom);
        tvCurrentSplit = findViewById(R.id.currentSplitCustom);
        tvAvgSplit = findViewById(R.id.avgSplitCustom);
        tvStrokeRate = findViewById(R.id.strokeRateCustom);

        btnStart = findViewById(R.id.btnStartCustom);
        btnStop  = findViewById(R.id.btnStopCustom);
        btnReset = findViewById(R.id.btnResetCustom);
        btnSave  = findViewById(R.id.btnSaveCustom);

        Intent intent = getIntent();
        String inputDistanceString = intent.getStringExtra("distance");
        if (inputDistanceString != null) {
            inputDistance = Double.parseDouble(inputDistanceString);  // assign to field, not new variable
            remainingDistance = inputDistance; // start countdown from full distance
        } else {
            inputDistance = 0;
            remainingDistance = 0;
        }
         
        remainingDistance = this.inputDistance;
    ...
    private void resetSession() {
        stopSession();
        totalDistance = 0;
        remainingDistance = inputDistance;
    ...
    private void updateButtons() {
        ...
        btnSave.setEnabled(!sessionRunning && (remainingDistance <= 0));
    }
    ...
    private void handleNewLocation(Location location) {
        if (lastLocation != null) {
            ...
            if (segment < 50.0) {
                ...
                remainingDistance = inputDistance - totalDistance;
    ...
    private void updateMetricsFromLocation(Location location) {
        ...
        if (lastLocation == null) {
            ...
            tvDistance.setText(String.format("%.1f m", remainingDistance));
